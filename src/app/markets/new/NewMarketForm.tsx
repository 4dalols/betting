    ...
    <form action={formAction} className="space-y-5">
      <fieldset disabled={pending} className="contents">
        <label className="block">
          ...
          <input name="question" required className="input" placeholder="Will it rain on Saturday?" />
        </label>

        <label className="block">
          <span className="mb-1 block text-sm text-zinc-400">Details (optional)</span>
          <textarea name="description" rows={3} className="input" placeholder="How this resolves, sources, etc." />
        </label>
        ...
        {type === "BINARY" ? (
          <div className="grid grid-cols-2 gap-3">
            <label className="block">
              ...
              <input name="yesLabel" defaultValue="Yes" className="input" />
            </label>
            <label className="block">
              <span className="mb-1 block text-sm text-zinc-400">Second option</span>
              <input name="noLabel" defaultValue="No" className="input" />
            </label>
          </div>
        ) : (
          <div>
            ...
            <div className="space-y-2">
              {Array.from({ length: outcomeCount }).map((_, i) => (
                <input key={i} name="outcomes" className="input" placeholder={`Choice ${i + 1}`} required={i < 2} />
              ))}
            </div>
            <div className="mt-2 flex gap-2 text-sm">
              <button type="button" className="btn-ghost" onClick={() => setOutcomeCount((c) => Math.min(12, c + 1))}>
                + Add choice
              </button>
              {outcomeCount > 2 && (
                <button type="button" className="btn-ghost" onClick={() => setOutcomeCount((c) => c - 1)}>
                  Remove last
                </button>
              )}
            </div>
        ...
        <label className="block">
          ...
          <input name="stake" type="number" min="0.01" step="0.01" required className="input" placeholder="5.00" />
